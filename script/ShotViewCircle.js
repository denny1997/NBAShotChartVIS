...
function ShotViewCircle() {
    ...
    self.init = function (maxHeight)
    {
        // creat SVG elements
	    self.hidden = false;
	    self.div = d3.select('#shotChartCircle');
        ...
        self.graph1 = self.svg.append('image');
		self.grpPlot1 = self.svg.append('g').attr('id','shotPlotCircle');
		self.graph2 = self.svg.append('image');
		self.grpPlot2 = self.svg.append('g').attr('id','shotPlotHex');
	    // // right bar plot
        ...
        var div   = document.getElementById('shotChartCircle');  // shortcuts
		...
		self.svg.append('text').attr('id','title-ShotViewCircle');
    ...
    self.update = function ()
    {
	    ...
	    var yearFrom = globData.currPlayerFilter.YearFrom ?
			globData.currPlayerFilter.YearFrom : player.info.FROM_YEAR;
	    var yearTo   = globData.currPlayerFilter.YearTo ?
			globData.currPlayerFilter.YearTo : Math.min(player.info.TO_YEAR,2015);
	    var attrTitle = 'Shooting Chart';
	    // window ratio
	    var ratio = self.svgW / 1520;
        // plot court
	    var barXheight = 50 * ratio,
        ...
        self.graph1
			...
			.style('opacity', 0.8);
		self.graph2
            .attr('x',imgX + imgW + 10 * ratio).attr('y',imgY)
            .attr('width',imgW).attr('height',imgH)
            .attr('xlink:href', 'data/halfCourt.png')
            .style('opacity', 0.8);
        // process dataset
		...
		console.log(yearTo)
		console.log(globData.currPlayerFilter.YearFrom)
		console.log(globData.currPlayerFilter.YearTo)
	    ...
	    self.svg.select('#title-ShotViewCircle')
		    .attr('x', self.svgW/2)
		    .attr('y', 40 * ratio)
	    ...
	    self.svg.append('defs').append("clipPath").attr("id", "clip")
		    ...
		    .attr("height", imgH);
        self.grpPlot2
	        .attr('transform', 'translate(' + (imgX + imgOX + imgW + 10 * ratio) + ',' + (imgY+imgOY) + ')')
		...
		let circle = self.grpPlot1.selectAll('cricle').data(rowpoint).enter().append('circle')
		...
		var groupCircleLegend = self.grpLegend.append('g');
	    ...
	    if (rowpoint.length > 0) {
		    ...
		    d3SelectAll(groupBarLegend, 'rect', FGPCTscale.range())
			    .attr('x', function (d, i) {
				    return imgX + 2 * imgW - 95 * ratio + (i - 3) * legendW;
			    })
			    .attr('y', imgY + imgH - 70 * ratio)
		    ...
		    groupHexLegend.selectAll(".hexagon")
			    ...
			    .attr("transform", function (d, i) {
				    var dx = imgX + 2 * imgW - 95 * ratio + (i - 2.5) * legendW,
					    dy = imgY + imgH - 30 * ratio;
				    return "translate(" + dx + "," + dy + ")";
			    });
		    d3SelectAll(groupHexLegend, 'text', hexRange)
			    .attr('x', function (d, i) {
				    return imgX + 2 * imgW - 95 * ratio + (i - 2.5) * legendW;
			    })
			    .attr('y', imgY + imgH - 10 * ratio)
			    .text(function (d) {
				    return d.toFixed(0);
			    })
			    .classed('legend-ShotView', true)
